...
void DmaDoneHandler(unsigned int Channel, XDmaPs_Cmd *DmaCmd,
	...
	DMA_Done = true;
	nextBuffer();
	//Xil_DCacheFlushRange((u32) &pixel_buffer, ((Width()*Height()) * sizeof(u32)));
}



Matrix_Driver::Matrix_Driver(u16 width = 64, u16 height = 32) {
	_width = width;
	_height = height;
...
void Matrix_Driver::begin() {
	...
	Dma_Cmd.BD.Length = (Width()*Height()) * sizeof(u32);
...
void Matrix_Driver::setPixel(u32 x, u32 y, u8 red, u8 green, u8 blue) {
	if(gamma_correction_enabled)
		pixel_buffer[x + (y*Width())] = (u32) ((getBuffer() << 24) | (gamma_lut[red] << 16) | (gamma_lut[green] << 8) | gamma_lut[blue]);
	else
		pixel_buffer[x + (y*Width())] = (u32) ((getBuffer() << 24) | (red << 16) | (green << 8) | blue);
}
